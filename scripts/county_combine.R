...
for (year in PURYr$year) {
  ...
  prodType_list[[glue::glue("prodType_", year = year)]] <- merged_data

  # Combine all years into a single data frame
  prodType_list$prodType_2022$full_exp_dt <- as.character(prodType_list$prodType_2022$full_exp_dt) # Was not merging bc wrong type. Only for these two years
  prodType_list$prodType_2021$full_exp_dt <- as.character(prodType_list$prodType_2021$full_exp_dt)
  combined_prodType <- bind_rows(prodType_list, .id = "yr")
